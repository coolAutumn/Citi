...
public abstract class GenericEntityDao<T, I extends Serializable> {
  ...
  /*  public
    ...
    /**
     * 通过属性查找List,并且按某一字段排序
     * @param propertyName
     * @param value
     * @param orderParamName
     * @param orderType
     * @param <M>
     * @return
     */
    public <M extends T> List<M> findByStringPropertyOrderBy(String propertyName, String value, String orderParamName, String orderType)
    {
        String queryString = "from " + entityClass().getName()
                + " as model where model." + propertyName + "= :PROPERTY_VALUE order by model." + orderParamName + orderType ;
        Query query = getCurrentSession().createQuery(queryString).setString("PROPERTY_VALUE",value);
        List list = query.list();
        return list;
    }
